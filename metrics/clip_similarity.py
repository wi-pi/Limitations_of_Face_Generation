"""
Adapted from https://github.com/timothybrooks/instruct-pix2pix/blob/main/metrics/clip_similarity.py
Class for computing CLIP metrics for a source, target image pair.
"""
...
class ClipSimilarity(nn.Module):
    def __init__(self, name: str = "ViT-L/14", device="cuda:1", download_path="./"):
        super().__init__()
        assert name in ("RN50", "RN101", "RN50x4", "RN50x16", "RN50x64", "ViT-B/32", "ViT-B/16", "ViT-L/14",
                        "ViT-L/14@336px")  # fmt: skip
        self.size = {"RN50x4": 288, "RN50x16": 384, "RN50x64": 448, "ViT-L/14@336px": 336}.get(name, 224)

        self.model, self.preprocess = clip.load(name, device=device, download_root=download_path)
    ...
    def forward(
        ...
        #print(sim_0, sim_1, sim_direction, sim_image)
        return sim_0, sim_1, sim_direction, sim_image

    def just_clip_scores(self, image_0 : torch.Tensor, text_0: list[str]):
        image_features_0 = self.encode_image(image_0)
        text_features_0 = self.encode_text(text_0)
        sim_0 = F.cosine_similarity(image_features_0, text_features_0)
